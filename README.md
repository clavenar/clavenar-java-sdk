...
```bash
base=https://github.com/clavenar/clavenar-java-sdk/releases/download/v1.5.5
curl -fsSLO "$base/agent-sdk-1.5.5.jar"
curl -fsSLO "$base/pom.xml"
curl -fsSLO "$base/bom.json"
curl -fsSLO "$base/bom.xml"
jar tf agent-sdk-1.5.5.jar >/dev/null
```
...
```kotlin
implementation("com.clavenar:agent-sdk:1.5.5")
```
...
multi-tool turns use one ordered atomic decision. Proxy 1.0.0 and Lite 1.0.0
